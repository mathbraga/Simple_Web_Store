...
const db = new sqlite3.Database(
    path.resolve(__dirname, './database.sqlite'), (err) => {
      if (err) {
...
const initialize = function initDb() {
  db.serialize(() => {
    ...
    db.run(`insert or ignore into categories (category) 
      values ('motherboards')`);
    db.run(`insert or ignore into categories (category) 
      values ('graphicscard')`);
    db.run(`insert or ignore into categories (category) 
    ...
    db.run(`insert or ignore into roles (id, role) 
      values (1, 'admin')`);
    db.run(`insert or ignore into roles (id, role) 
      values (2, 'colaborator')`);
    db.run(`insert or ignore into roles (id, role) 
      values (3, 'customer')`);

    db.run(`insert or ignore into users 
      (role_id, full_name, address, email, password) 
      values 
      (1, 'admin', 'unknown', 'admin@admin.com', 'admin123')`);
